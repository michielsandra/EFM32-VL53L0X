...
// from the I2C bus
//
// adapted library from Larry Bank:
...
static unsigned short readReg16(uint8_t ucAddr) {
	uint8_t ucTemp[2];
	int rc;
	rc = IIC_WriteBuffer(VL53L0X_I2C_ADDR, &ucAddr, 1);
	if (rc == 1) {
		rc = IIC_ReadBuffer(VL53L0X_I2C_ADDR, ucTemp, 2);
	}
...
static unsigned char readReg(uint8_t ucAddr) {
	...
	rc = IIC_WriteBuffer(VL53L0X_I2C_ADDR, &ucAddr, 1);
	if (rc == 1) {
		rc = IIC_ReadBuffer(VL53L0X_I2C_ADDR, &ucTemp, 1);
		if (rc != 1) {
		};
	}
	return ucTemp;
...
static void readMulti(uint8_t ucAddr, uint8_t *pBuf, uint8_t iCount) {
	int rc;
	rc = IIC_WriteBuffer(VL53L0X_I2C_ADDR, &ucAddr, 1);
	if (rc == 1) {
		rc = IIC_ReadBuffer(VL53L0X_I2C_ADDR, pBuf, iCount);
		if (rc != iCount) {
		};
	}
} /* readMulti() */

static void writeMulti(uint8_t ucAddr, uint8_t *pBuf, uint8_t iCount) {
	unsigned char ucTemp[16];
	int rc;
	ucTemp[0] = ucAddr;
	memcpy(&ucTemp[1], pBuf, iCount);
	rc = IIC_WriteBuffer(VL53L0X_I2C_ADDR, ucTemp, iCount + 1);
	if (rc != iCount + 1) {
	};
} /* writeMulti() */
...
static void writeReg16(uint8_t ucAddr, unsigned short usValue) {
	...
	rc = IIC_WriteBuffer(VL53L0X_I2C_ADDR, ucTemp, 3);
	if (rc != 3) {
	}; // suppress warning
} /* writeReg16() */
...
static void writeReg(uint8_t ucAddr, uint8_t ucValue) {
	...
	ucTemp[0] = ucAddr;
	ucTemp[1] = ucValue;
	rc = IIC_WriteBuffer(VL53L0X_I2C_ADDR, ucTemp, 2);
	if (rc != 2) {
	}; // suppress warning
} /* writeReg() */
...
static void writeRegList(uint8_t *ucList) {
	...
	while (ucCount) {
		rc = IIC_WriteBuffer(VL53L0X_I2C_ADDR, ucList, 2);
		if (rc != 2) {
		};
		ucList += 2;
		ucCount--;
...
unsigned char ucI2CMode[] =
		{ 4, 0x88, 0x00, 0x80, 0x01, 0xff, 0x01, 0x00, 0x00 };
unsigned char ucI2CMode2[] = { 3, 0x00, 0x01, 0xff, 0x00, 0x80, 0x00 };
unsigned char ucSPAD0[] = { 4, 0x80, 0x01, 0xff, 0x01, 0x00, 0x00, 0xff, 0x06 };
unsigned char ucSPAD1[] = { 5, 0xff, 0x07, 0x81, 0x01, 0x80, 0x01, 0x94, 0x6b,
		0x83, 0x00 };
unsigned char ucSPAD2[] = { 4, 0xff, 0x01, 0x00, 0x01, 0xff, 0x00, 0x80, 0x00 };
unsigned char ucSPAD[] = { 5, 0xff, 0x01, 0x4f, 0x00, 0x4e, 0x2c, 0xff, 0x00,
		0xb6, 0xb4 };
unsigned char ucDefTuning[] = { 80,  };
...
static void getSequenceStepTimeouts(uint8_t enables,
		SequenceStepTimeouts * timeouts) {
	timeouts->pre_range_vcsel_period_pclks = ((readReg(
			PRE_RANGE_CONFIG_VCSEL_PERIOD) + 1) << 1);
	...
	timeouts->pre_range_mclks = decodeTimeout(
			readReg16(PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI));
	timeouts->pre_range_us = timeoutMclksToMicroseconds(
			timeouts->pre_range_mclks, timeouts->pre_range_vcsel_period_pclks);

	timeouts->final_range_vcsel_period_pclks = ((readReg(
			FINAL_RANGE_CONFIG_VCSEL_PERIOD) + 1) << 1);
...
//		return 0;
	...
	if (model) {
		...
		i = IIC_WriteBuffer(VL53L0X_I2C_ADDR, ucTemp, 1);
		//i = read(file_i2c, ucTemp, 1);
		i = IIC_ReadBuffer(VL53L0X_I2C_ADDR, ucTemp, 1);
		if (i == 1)
			*model = ucTemp[0];
	}
	if (revision) {
